...
int offsetOfParameter = 16; // change from 8 to 16
...
void declareIdList(AST_NODE* declarationNode, SymbolAttributeKind isVariableOrTypeAttribute, int ignoreArrayFirstDimSize)
{
    ...
    while(traverseIDList)
    {
        ...
        {
            // attribute used for enter symboltable
            ...
            {
            case NORMAL_ID:
                // typeDescriptorOfTypeNode ~ typeNode->semantic_value.identifierSemanticValue.symbolTableEntry->attribute->attr.typeDescriptor
                // copy pointer?
                ...
                break;
            // offset now don't deal with array in phase 5
            case ARRAY_ID:
            ...
            else
            {
                ...
                {
                    // for parameters
                    attribute->offsetInAR = offsetOfParameter;
                }
            }
        }
...
// represents assign_expr( relop_expr + assign_stmt_node + function_call )
void checkAssignOrExpr(AST_NODE* assignOrExprRelatedNode)
{
...
// system call with only 1 parameter!
void checkWriteFunction(AST_NODE* functionCallNode)
{
...
void checkFunctionCall(AST_NODE* functionCallNode)
{
    ...
    AST_NODE* actualParameter = actualParameterList->child;
    // this is for prototype. ex: void func(int a, int b, int c)
...
// actualParameter.dataType = ERROR_TYPE when error occurs
void checkParameterPassing(Parameter* formalParameter, AST_NODE* actualParameter)
{
...
// deal with expr_node, function_call, identifier_ndoe, const_calue_node
void processExprRelatedNode(AST_NODE* exprRelatedNode)
{
...
void evaluateExprValue(AST_NODE* exprNode)
{
    if(exprNode->semantic_value.exprSemanticValue.kind == BINARY_OPERATION)
    {
        ...
        {// leftOp.dataType = FLOAT_TYPE or rightOp.dataTyoe = FLOAT_TYPE
            float leftValue = 0;
            float rightValue = 0;
    ...
    if(exprNode->semantic_value.exprSemanticValue.kind == BINARY_OPERATION)
    {
        ...
        if(exprNode->dataType != ERROR_TYPE)
        {
            ...
            case BINARY_OP_OR:
                ...
                break;
            default:
                // suppose type converting is enforced at code-gen stage
                exprNode->dataType = getBiggerType(leftOp->dataType, rightOp->dataType);
                break;
            }
        }

        // constant folding, when leftnode and rightnode are both const(const_value_node or expr_node is const)
        if((exprNode->dataType != ERROR_TYPE) &&
...
// here is the place generate INT_PTR OR FLOAT_PTR
void processVariableRValue(AST_NODE* idNode)
{
    SymbolTableEntry *symbolTableEntry = retrieveSymbol(idNode->semantic_value.identifierSemanticValue.identifierName);
...
void checkReturnStmt(AST_NODE* returnNode)
{
    ...
    while(parentNode)
    {
        // backtrack to decl_node(function_decl)
...
void processBlockNode(AST_NODE* blockNode)
{
    // calculate offset for local varibles
    int previousAROffset = g_offsetInARAux;
    // open and close scope for blocknode
    openScope();
    ...
    // may result bugs
    if (g_deepestBlockVariableOffset > g_offsetInARAux) {
        g_deepestBlockVariableOffset = g_offsetInARAux;
    }
    g_offsetInARAux = previousAROffset;

    closeScope();
    
}

// stmt includes NULL_NODE, BLOCKNODE and while, for, assign, if, func_call, return
void processStmtNode(AST_NODE* stmtNode)
{
...
void declareFunction(AST_NODE* declarationNode)
{
    ...
    // open scope for parameterlist and following block
    openScope();
    ...
    offsetOfParameter = 16; // change from 8 to 16
    ...
    if(!errorOccur)
    {
        ...
        // select minimum because g_offsetInARAux & g_deepestBlockVariableOffset are non-positive!!!
        attribute->offsetInAR = (g_offsetInARAux > g_deepestBlockVariableOffset)? g_deepestBlockVariableOffset : g_offsetInARAux;
    }
